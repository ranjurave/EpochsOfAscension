...
#include "ShuttleQuaternions.h"

// Formula to convert a Euler angle in degrees to a quaternion rotation
FQuat UShuttleQuaternions::Euler_To_Quaternion(FRotator Current_Rotation) {
...
void UShuttleQuaternions::SetWorldRotationQuat(USceneComponent* SceneComponent, const FQuat& Desired_Rotation) {
    if (SceneComponent) {
...
void UShuttleQuaternions::SetRelativeRotationQuat(USceneComponent* SceneComponent, const FQuat& Desired_Rotation) {
    if (SceneComponent) {
...
void UShuttleQuaternions::AddLocalRotationQuat(USceneComponent* SceneComponent, const FQuat& Delta_Rotation) {
    if (SceneComponent) {
...
void UShuttleQuaternions::SetActorWorldRotationQuat(AActor* Actor, const FQuat& Desired_Rotation) {
    if (Actor) {
...
void UShuttleQuaternions::SetActorRelativeRotationQuat(AActor* Actor, const FQuat& Desired_Rotation) {
    if (Actor) {
...
void UShuttleQuaternions::AddActorLocalRotationQuat(AActor* Actor, const FQuat& Delta_Rotation) {
    if (Actor) {
